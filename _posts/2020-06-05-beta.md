...
Now we have the value-weighted factor having positive mean return, Sharpe ratio, and alpha in every decade.  So betting against beta seems to have been consistently protitable.  But, for this to be true, we need to make the beta one adjustment.

# Embedded Options

The explanation I put forth in the paper was that the the time-varying leverage makes the portfolio look like an option-writing strategy.  
