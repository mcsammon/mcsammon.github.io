...
Letting $$\lambda$$ be the multiplier on the BC, taking first order conditions we get:
\begin{equation}
\begin{split}
\text{[c]} \quad \quad  c^{-\frac{1}{\gamma}}=\lambda \\
\text{[k']}\quad \beta v'(k') = \lambda \\
\text{[Envelope]}\quad v'(k)= \lambda
\end{split}
\end{equation}
...
\end{equation}
Using our formula for optimal consumption we get:
\begin{equation}
\begin{split}
c_0=(1-\beta^{\gamma}) k_0 \\
c_t=\beta^\gamma c_{t-1} \text{ for all $$t>0$$}
\end{split}
\end{equation}
