...
\end{equation}
Now consider the ex-post rational price, $$P^\*$$ - the price if there was no uncertainty about future dividends:
\begin{equation}
P_t^*=\sum \limits_{i=1}^{\infty} \left(\frac{1}{1+r} \right)^i d_{t+i}
\end{equation}
...
Using this we get:
\begin{equation}
P_t^\*=P_t+\sum \limits_{i=1}^{\infty} \left(\frac{1}{1+r} \right)^i \epsilon_{t+i}
\end{equation}
...
\begin{equation}
V(P_t^*)=V(P_t)+ K V(\epsilon_t) \geq V(P_t)
\end{equation}
...
1) We don't actually observe $$P^\*$$ so we need to set a terminal value (for example, use todays price and construct $$P^*$$ recursively using observed dividends).  This introduces measurement error that can bias the test results. <br />
